...
module.exports.showListing = async (req, res) => {
  const listing = await Listing.findById(req.params.id)
    ...
    }) // Populate the author field of reviews
    .populate("owner");
  // Check the structure
  if (!listing) {
...
module.exports.createListing = async (req, res, next) => {
  try {
    const location = await geocodeWithMapTiler(req.body.listing.location);
...
module.exports.renderEditForm = async (req, res) => {
  ...
  let originalImageUrl = listing.image.url; // Store the original image
